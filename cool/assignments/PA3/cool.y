...
/* Feature list may not be empty, no empty features in list. */
...
| OBJECTID ':' TYPEID ASSIGN expression                   /* attribute with value */
{
  $$ = attr($1, $3, $5);
}
| OBJECTID '(' ')' ':' TYPEID '{' expression '}'          /* method without input */
{
  $$ = method($1, nil_Formals(), $5, $7);
}
| OBJECTID '(' formal_list ')' ':' TYPEID '{' expression '}'    /* method */
{
  $$ = method($1, $3, $6, $8);
}
;
...
/* expression list separated by semicolon */
...
conditional: IF expression THEN expression FI
{
  $$ = cond($2, $4, no_expr());
}
